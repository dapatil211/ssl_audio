...
def pitch_shift_audio(clip, sample_rate, return_audio=False):
    ...
    clip_pitch_shifted = librosa.effects.pitch_shift(
        clip, sample_rate, n_steps=num_steps, bins_per_octave=num_bins
    )  # shifted by 4 half steps # maybe [-15,15] avoid 0, bins in [5,24]
    if return_audio:
...
def time_stretch_audio(clip, return_audio=False):
    speed_factor = random.uniform(
        0.7, 1.3
    )  # make sure it is not 1. If < 1, makes the sound deeper, if > 1 creates higher frequencies
...
def spectral_distorsion_model_audio(clip, return_audio=False):
    ...
    PDM = np.array(
        [np.complex(np.cos(p), np.sin(p)) for p in np.random.normal(0, sigma, size=513)]
    )
...
def LPC_synthesis_audio2(clip, return_audio=False):
    order = random.randint(
        2, 16
    )  # advice on how to choose order: https://stackoverflow.com/questions/61519826/how-to-decide-filter-order-in-linear-prediction-coefficients-lpc-while-calcu
...
# Another smooth with Fourrier Transform
# Less obvious if it is really useful or what it does exactly ??
...
def smooth_convolve_audio(clip, return_audio=False):
    ...
    if random.randint(0, 2) == 0:
        kernel = gaussian(
            window_size, 1
        )  # not really apparent since very small, use with wrap and reflect to amplify maybe
...
def phase_vocoder_audio(clip, return_audio=False):
    ...
    speed_rate = random.uniform(0.3, 2.0)  # Maybe remove 1.
    clip_speed = librosa.phase_vocoder(
        D, rate=speed_rate, hop_length=512
    )  # rate in [0.3, 2.]
...
def speed_tune_audio(clip, sample_rate, return_audio=False):
    ...
    if len(wav_speed_tune) < sample_rate:  # sample_rate=16000
        ...
        wav_speed_tune = wav_speed_tune[
            int(cut_len / 2) : int(cut_len / 2) + sample_rate
        ]
    if return_audio:
...
def mix_audios(clip1, clip2, return_audio=False):
    ...
    if (
        clip2.shape[0] - min_length == 0
    ):  # if clip2(=background) is shorter, choose start_=0 to avoid error with np.random.randint
        start_ = 0
    else:
        start_ = np.random.randint(clip2.shape[0] - min_length)
    clip2_slice = clip2[start_ : start_ + min_length]
    clip_with_bg = clip1[:min_length] * np.random.uniform(0.8, 1.2) + clip2_slice[
        :min_length
    ] * np.random.uniform(
        0.1, 0.8
    )  # vary values
...
def spectral_subtraction_audio(clip, return_audio=False):
    denoised_signal = pyroomacoustics.denoise.spectral_subtraction.apply_spectral_sub(
        noisy_signal=clip, nfft=512, db_reduc=10, lookback=5, beta=20, alpha=3
    )
    if return_audio:
...
def iterative_wiener_audio(clip, return_audio=False):
    denoised_signal = pyroomacoustics.denoise.iterative_wiener.apply_iterative_wiener(
        noisy_signal=clip,
        frame_len=512,
        lpc_order=20,
        iterations=2,
...
def subspace_audio(clip, return_audio=False):
    denoised_signal = pyroomacoustics.denoise.subspace.apply_subspace(
        noisy_signal=clip, frame_len=256, mu=10, lookback=10, skip=2, thresh=0.01
    )
    if return_audio:
        return denoised_signal
    else:
...
def Griffin_Lim_vocoder(clip, return_audio=False):
    ...
    griffin_lim_clip = pyroomacoustics.phase.griffin_lim(
        X_mag, hop, win_a, n_iter=n_iter
    )
    if return_audio:
...
def sequencial_transforms(
    clip, sample_rate, min_transforms=2, max_transforms=6, include_sinc=True
):  # 20 Transforms in total
    transforms_list = [
        "compand",
        "fade",
        "pitch",
    ...
    while num_transforms > 0:
        ...
        if transform == 1:
            ...
            if "bass" in transforms_list:
                ...
                slope = random.uniform(0.3, 1.0)
                tfm.bass(
                    gain_db=gain_db, frequency=100.0, slope=slope
                )  # slope in [0.3, 1.], gain_db in [-20,20]
            ...
            if "bend" in transforms_list:
                ...
                time_points = np.random.uniform(
                    low=0.1, high=audio_duration, size=num_bends * 2
                )
            ...
            if "chorus" in transforms_list:
                ...
                tfm.chorus(
                    gain_in=0.5, gain_out=0.9, n_voices=n_voices
                )  # n_voices in [2,5]
            ...
            if "treble" in transforms_list:
                ...
                gain_db = random.randint(-20, 20)
                tfm.treble(
                    gain_db=gain_db, frequency=frequency, slope=slope
                )  #  slope in [0.3,1.0], frequency in [100, 3000.], db in [-20,20]
            ...
            if "tempo" in transforms_list:
                ...
                quick = quick[random.randint(0, 1)]
                tfm.tempo(
                    factor=factor, audio_type="s", quick=quick
                )  # factor in [0.1,1.9], quick in [True, False]
            ...
            if "overdrive" in transforms_list:
                colour = random.uniform(1.0, 20.0)
                gain_db = random.uniform(1.0, 100.0)
            ...
            if "phaser" in transforms_list:
                gain_in = random.uniform(0.1, 1.0)
                ...
                tfm.phaser(
                    ...
                    decay=decay,
                    speed=speed,
                    modulation_shape=modulation_shape,
                )  # gain in [0.1, 1.], modulation_shape in ['triangular', 'sinusoidal'], speed in [0.1, 2], decay in [0.1,0.5], delay in [1,5]
            ...
            if "reverb" in transforms_list:
                ...
                tfm.reverb(
                    ...
                    wet_gain=wet_gain,
                )  # Very good. reverberance in [20,60], high_freq_damping in [30, 70], room_scale in [20, 100], stereo_depth in [20, 100], wet_gain in [1,20]
...
def apply_sequential_transforms(
    clip, sample_rate, min_transforms=2, max_transforms=6, return_audio=False
):
    Done = False
    include_sinc = True  # sometimes problematic
    while Done != True:
        try:
            y = sequencial_transforms(
                clip,
                sample_rate,
                min_transforms=min_transforms,
                max_transforms=max_transforms,
                include_sinc=include_sinc,
            )
            Done = True
        except:
            include_sinc = False
...
# add random line
def draw_line(
    x, length=[5, 20], thickness_length=[2, 4]
):  # works with an image as input
    result = np.copy(x)
    width = x.shape[1]
    height = x.shape[0]
    ...
    cv2.line(
        result,
        (x1, y1),
        (int(np.min([width, x2])), int(np.min([height, y2]))),
        color1,
        thickness,
    )
...
def butterworth_sound_filter(clip, sample_rate, return_audio=False):
    ...
    if rand == 1:
        b, a = signal.butter(
            5, 1000 / (sample_rate / 2), btype="highpass"
        )  # ButterWorth filter 4350
        filteredSignal = signal.lfilter(b, a, clip)
    elif rand == 2:
        c, d = signal.butter(
            5, 380 / (sample_rate / 2), btype="lowpass"
...
def mel_to_stft_griffinLim(clip, sample_rate, return_audio=False):
    melspectrogram = librosa.feature.melspectrogram(
        y=clip, sr=sample_rate, window="hanning"
