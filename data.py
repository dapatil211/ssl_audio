...
# def get_self_supervised_data(dataset=constants.Dataset.LBS, shuffle_buffer=1000):
...
#     ds_train = tfds.load(dataset.value, split=split, as_supervised=True)
...
# def get_downstream_dataset(dataset=constants.Dataset.VOXFORGE, shuffle_buffer=1000):
...
#     ds_train = ds_train.shuffle(shuffle_buffer, reshuffle_each_iteration=True).map(
#         _parse_example, num_parallel_calls=tf.data.experimental.AUTOTUNE
#     )

#     ds_test = ds_test.shuffle(shuffle_buffer, reshuffle_each_iteration=True).map(
#         _parse_example, num_parallel_calls=tf.data.experimental.AUTOTUNE
#     )
...
def create_spectogram(x, sr=None, n_frames=98, is_training=False):
